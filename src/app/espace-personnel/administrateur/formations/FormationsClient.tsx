...
import {
	...
	toggleTrainingVisibilityAction,
	updateTrainingAction,
} from "@/lib/server/actions/admin";
import type { ActionState } from "@/lib/server/actions/types";
import type { AdminTraining } from "@/lib/types";
...
/**
 * Formations et thèmes viennent du serveur. La recherche, le filtre par thème
 * et l'affichage des masquées restent en local : le catalogue tient en quelques
 * dizaines d'entrées, filtrer côté client évite un aller-retour par frappe.
 */
export default function FormationsClient({ trainings, themes }: FormationsClientProps) {
	...
	const handleSave = async (e: React.FormEvent) => {
		...
		try {
			...
			await run(
				() => (editingId ? updateTrainingAction(editingId, payload) : createTrainingAction(payload)),
				editingId ? "Formation modifiée avec succès." : "Formation créée avec succès.",
				() => setModalOpen(false)
			);
		} finally {
			setSaving(false);
	...
	const handleDelete = async () => {
		...
		setDeleting(true);
		try {
			await run(() => deleteTrainingAction(deleteId), "Formation supprimée.", () => setDeleteId(null));
		} finally {
			setDeleting(false);
		}
	};

	const handleToggleVisibility = (training: Training) => {
		const newValue = training.isVisible === false;
		return run(() => toggleTrainingVisibilityAction(training._id, newValue), `Formation ${newValue ? "visible" : "masquée"}.`);
	};
